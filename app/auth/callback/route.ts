...
function getSafeNext(next: string | null) {
  if (!next || !next.startsWith("/")) return "/profile/me";
  if (next.startsWith("//")) return "/profile/me";
  return next;
}
...
export async function GET(request: NextRequest) {
  ...
  if (role === "admin" || role === "owner") {
    ...
    if (loginLogError) {
      console.error(
        "[auth/callback] Falha ao registrar log de login administrativo:",
        loginLogError,
      );
    }
  }
  ...
  if (typeof accessToken === "string" && accessToken.length > 0) {
    try {
      ...
      console.error(
        "[auth/callback] Falha ao sincronizar conexoes do Discord:",
        connectionsError,
      );
    }
  }
