...
function StatCardSkeleton() {
  return (
    <div className="h-[120px] rounded-xl border border-white/5 bg-white/5 p-5 animate-pulse">
...
function ActivitySkeleton() {
  return (
    <div className="h-[250px] rounded-xl bg-white/5 p-4 animate-pulse">
...
async function HeaderSection({ displayName }: { displayName: string }) {
  const now = new Date();
    ...
    <header className="admin-surface rounded-2xl p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-bold text-[color:var(--text-strong)]">
            {getGreetingLabel(now)}, @{displayName}
          </h1>
...
async function ChartsSection() {
  const [weeklyUsers, monthlyTeams, statsPayload] = await Promise.all([
    getWeeklyUsers(),
    getMonthlyTeams(),
    getDashboardStats(),
  ]);
  ...
  return (
    <DashboardCharts
      ...
      monthlyTeams={monthlyTeams}
      tournamentStatus={statsPayload.tournamentStatus}
    />
  );
}
  ...
  return (
    <section className="admin-surface space-y-4 rounded-2xl p-6">
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        ...
        <AlertBanner
          severity={
            alerts.securityAuthFailures15m >= 8
              ? "error"
          ...
          description={
            alerts.securityAuthFailures24h > 0
              ? `${numberFmt.format(alerts.securityAuthFailures24h)} falhas nas últimas 24h (${numberFmt.format(
                  alerts.securityAuthFailures15m,
                )} nos últimos 15 min).`
        ...
        <AlertBanner
          severity={
            alerts.criticalAdminActions24h > 0
              ? "error"
              : alerts.suspiciousAdminActions24h > 0
              ? "warning"
              : "info"
          }
          ...
          description={
            alerts.criticalAdminActions24h > 0 || alerts.suspiciousAdminActions24h > 0
              ? `${numberFmt.format(alerts.criticalAdminActions24h)} críticas e ${numberFmt.format(
                  alerts.suspiciousAdminActions24h,
                )} suspeitas nas últimas 24h.`
  ...
  return (
    <section className="admin-surface space-y-4 rounded-2xl p-6">
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <div className="admin-surface-muted space-y-2 rounded-xl p-4">
          ...
          <div className="space-y-2">
            {activity.latestUsers.length > 0 ? (
              ...
              <p className="rounded-lg border border-slate-200 dark:border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-500 dark:text-slate-400">
                Sem registros.
              </p>
            )}
          </div>
        ...
        <div className="admin-surface-muted space-y-2 rounded-xl p-4">
          <p className="text-xs uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">
            Ultimas 5 acoes administrativas
          </p>
          <div className="space-y-2">
...
export default async function AdminDashboardPage() {
  ...
  return (
    <section className="space-y-6">
      <Suspense
        fallback={
          <header className="admin-surface rounded-2xl p-6 h-28 animate-pulse bg-white/5" />
        }
      >
      ...
      <Suspense
        fallback={
          <section className="admin-surface rounded-2xl p-6 h-64 animate-pulse bg-white/5" />
        }
      >
        <AlertsSection />
      </Suspense>
