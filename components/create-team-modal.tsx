  ...
  return (
    ...
    >
      ...
      <div className="relative flex w-full max-w-xl flex-col rounded-3xl border border-white/10 bg-[#0d1f33] shadow-2xl">
        ...
        >
          <div className="space-y-5"> {!userXboxGamertag && ( <div> <label className="mb-1.5 block text-sm font-medium text-slate-200"> Sua Xbox Gamertag <span className="text-rose-400">*</span> <span className="text-xs text-amber-400 ml-2">(Apenas 1 vez)</span> </label> <input id="xbox_gamertag" name="xbox_gamertag" disabled={isPending || teamLimitReached} placeholder="Ex: PlayerOne#1234" className="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-slate-100 outline-none ring-amber-300/40 transition focus:ring disabled:opacity-50" required /> <p className="mt-1 text-xs text-slate-400"> Necess�rio para participar de equipes e torneios. Voc� n�o poder� alterar isso depois sem contatar a modera��o. </p> </div> )} 
            {/* ----- Nome da equipe ----- */}
            <div>
